...
def get_shape_value(x, y, estimator, ex_type="tree", cv=5):
    """
...
def shap_value_selection(x, y, estimator, rank, cv=5, ex_type='tree'):
    """
    ...
    shap_df = x.copy()
    shap = get_shape_value(x, y, estimator=estimator, cv=cv)
    shap_df.loc["shap_mean", :] = shap.loc["shap_mean", :].values
    shape_df = shap_df.sort_values("shap_mean", ascending=False, axis=1)
